<template>
  <div :ref="name" :class="{ 'dropdown-active': isShow }" class="dropdown-wrap">
...
export default defineComponent({
  ...
  computed: {
    getStylePosition() {
      ...
      return `top: ${Math.floor(this.top)}px; left: ${Math.floor(
        this.left
      )}px; max-width: ${this.contentMaxWidth}px;`;
    },
  },
  ...
  methods: {
    ...
    setEventsDropdown() {
      this.closeDropdown();
      window.__eventsDropdown[`${this.name}-box`] = this;
      this.showContent();
    },

    removeEventsDropdown() {
      delete window.__eventsDropdown[`${this.name}-box`];
    },
    ...
    async showContent() {
      ...
      const $element = this.$refs[this.name].querySelector(".dropdown-toggle");
      ...
      if (this.isShow) {
        this.$nextTick(() => {
          const $box = this.$refs[`${this.name}-box`];
    ...
    autoPosition(position, targetEl, boxEl) {
      ...
      if (
        ...
        clientHeight - targetEl.bottom < height + heightFooter
      )
